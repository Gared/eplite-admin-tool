...
public class SettingsModel {
	...
	public void connect() {
		try {
			...
			con = DriverManager.getConnection("jdbc:hsqldb:file:store.db",
					"SA", "");
	...
	public String[] getServerConfig() {
		...
		try {
			PreparedStatement pSt = con
					.prepareStatement("SELECT id,url,apiKey FROM serverSettings WHERE id = ?");
			pSt.setInt(1, 1);
			ResultSet resSet = pSt.executeQuery();
			if (resSet.next()) {
	...
	public void saveServerConfig(String url, String apiKey) {
		try {
			...
			PreparedStatement pSt = con
					.prepareStatement("INSERT INTO serverSettings (id, url, apiKey) VALUES (?, ?, ?)");
			pSt.setInt(1, 1);
			pSt.setString(2, url);
